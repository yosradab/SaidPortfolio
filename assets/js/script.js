(() => {
  ...
  const cardSections = document.querySelectorAll('main > section, main > #projects-section, main > #recommendations, footer');
  ...
  function updateRecede() {
    ...
    cardSectionsArr.forEach((sec, i) => {
      ...
      const isRec = sec.id === 'recommendations';
      ...
      if (isRec) {
        if (recScrollProgress < 0.99) { sec.style.filter = ''; return; }
        if (rect.bottom < vh && rect.bottom > 0) {
          const exitProgress = Math.min(1, 1 - rect.bottom / vh);
          const scale = 1 - exitProgress * 0.04;
          sec.style.transform = `translateY(${exitProgress * -24}px) scale(${scale})`;
  ...
  const recHints = document.querySelectorAll('.rec-hint');
  const recSlides = document.querySelectorAll('.rec-slide');

  const recHintEN = [...recHints].filter(h => h.classList.contains('lang-en'));
  const recHintFR = [...recHints].filter(h => h.classList.contains('lang-fr'));
  ...
  function setupRec() {
    if (!recScroll) return;
    recScroll.style.height = isMobile() ? '' : (RN * window.innerHeight) + 'px';
  }

  let lastRecIdx = -1;
  let _recWasAtEnd = false;

  function updateRec() {
    if (!recScroll || !recTrack) return;
    if (isMobile() || document.body.classList.contains('modal-open')) {
      if (!isMobile()) return;
